                ...
                <div class="h-full flex flex-col py-6 bg-white shadow-xl overflow-y-scroll">
                    <div class="px-4 sm:px-6">
                        <h2 class="text-lg font-bold text-white bg-gray-900" id="slide-over-title">
                            Editar Producto {{$productoedit->name}}
                        </h2>
                    </div>
                    <div class="mt-6 relative flex-1 px-4 sm:px-6">
                        <div class="absolute inset-0 px-4 sm:px-6">
                            <div class="bg-yellow-600 border-2 border-solid border-gray-600 py-5 px-2"
                                aria-hidden="true">
                                <form action="{{ route('editar.producto',$productoedit) }}" method="POST"
                                    enctype="multipart/form-data">
                                    <x-jet-validation-errors class="mb-4" />
                                    @csrf
                                    <label for="name" class="bg-gray-900 text-white w-full mb-2 px-2">Nombre</label>
                                    <input type="text" name="name" class="w-full mb-2" value="{{$productoedit->name}}" required>
                                    <label for="name" class="bg-gray-900 text-white w-full mb-2 px-2">Precio</label>
                                    <input type="number" name="precio" class="w-full mb-2" value="{{$productoedit->precio}}" required>
                                    <label for="name"
                                    ...
                                    <textarea name="descripcion" id="" cols="2" rows="3" class="w-full mb-2"
                                        required>{{$productoedit->descripcion}}
                                      </textarea>
                                    ...
                                    <label for="name" class="bg-gray-900 text-white w-full mb-2 px-2">Imagen actual</label>
                                    <img alt="avatar" class="w-3/4 md:w-32 h-24 rounded-xl my-2 md:my-0 mx-auto" src="{{ asset($productoedit->url) }}" />
                                    <label for="name" class="bg-gray-900 text-white w-full mb-2 px-2">Seleccionar una nueva imagen</label>
                                    <input type="file" name="logo" class="w-full mb-2" accept="image/*" >
      
                                    <button type="submit"
                                        class=" mt-4 m-auto bg-green-500 hover:bg-green-600 px-2 rounded-xl">GUARDAR CAMBIOS</button>
                                </form>
                            </div>
                        </div>
                    </div>
